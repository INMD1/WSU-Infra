  ...
  return (
    <Modal title="새 가상 머신 생성" onClose={onClose}>
      ...
      <form onSubmit={onSubmit}>
        {/* VM 이름 */}
        <label className="block text-sm font-medium text-body-strong mb-1">VM 이름</label>
        <input
        ...
        {/* 디스크 */}
        <label className="block text-sm font-medium text-body-strong mb-1">Disk (GB)</label>
  ...
  return (
    <div
      ...
      className={`relative p-3 rounded-md border-2 cursor-pointer transition-all flex flex-col justify-between min-h-[80px] ${
        isSelected ? 'border-primary bg-canvas shadow-sm' : 'border-hairline hover:border-primary/50 bg-surface-soft'
      }`}
    >
      ...
      {isSelected && (
        <div className="absolute -top-1.5 -right-1.5 bg-primary text-on-primary w-4.5 h-4.5 rounded-full flex items-center justify-center text-[10px] font-bold shadow-sm">
          ✓
        </div>
      )}
    </div>
